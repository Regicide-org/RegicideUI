...
public class ForwardItem extends PageItem {
    ...
    public ItemProvider getItemProvider(PagedGui<?> gui) {
        ...
        String nameText = "Следующая страница";
        ...
        if (gui.hasNextPage()) {
            m.setCustomModelData(119);
            lore.add(MiniMessage.miniMessage().deserialize("<i:false><white>Ԁ<dark_gray> > <#E9D282>Клик <#FCFC00>– перейти на <yellow>" + (gui.getCurrentPage() + 2) + "<#E9D282>/<yellow>" + gui.getPageAmount() + "</i>"));
        } else {
            m.setCustomModelData(123);
