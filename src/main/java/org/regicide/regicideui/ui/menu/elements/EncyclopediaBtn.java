...
public final class EncyclopediaBtn extends AbstractItem {
    ...
    public ItemProvider getItemProvider() {
        ...
        String titleText = RegicideUI.l().c().getString("menu-button-encyclopedia-name");
        Component title = MiniMessage.miniMessage().deserialize("<i:false><white>"+titleText+"</white></i>");
        m.displayName(title);



        List<String> loreText = RegicideUI.l().c().getStringList("menu-button-encyclopedia-lore");
    ...
    public void handleClick(@NotNull ClickType clickType, @NotNull Player player, @NotNull InventoryClickEvent event) {
        if (clickType.isLeftClick()) {
            ...
            Window window = Window.merged()
                    .setViewer(player)
                    .setGui(new HelpGUI(WindowManager.getInstance().getOpenWindow(player)).getGui())
